<x-app-layout>
    <!-- Page Title Starts -->
    <x-page-title page="Lista de Submenus" header="Lista de Submenus" />
    <!-- Page Title Ends -->
    ...
    <!-- submenu List Starts -->
    <div class="space-y-4">
        <!-- submenu Header Starts -->
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row md:gap-y-0">
            <!-- submenu Search Starts -->
            <form method="GET" action="{{ route('submenus.index') }}" class="group flex h-10 w-full items-center rounded-primary border border-transparent bg-white shadow-sm focus-within:border-primary-500 focus-within:ring-1 focus-within:ring-inset focus-within:ring-primary-500 dark:border-transparent dark:bg-slate-800 dark:focus-within:border-primary-500 md:w-72">
        ...
        <!-- submenu Table Starts -->
        <div class="table-responsive whitespace-nowrap rounded-primary">
            <table class="table">
                <thead>
                    <tr>
                        <th class="w-[5%]">
                            <input class="checkbox" type="checkbox" data-check-all data-check-all-target=".submenu-checkbox" />
                        </th>
                ...
                <tbody>
                    @foreach($submenus as $submenu)
                        <tr>
                            <td>
                                <input class="checkbox submenu-checkbox" type="checkbox" />
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    <div class="dropdown" data-placement="bottom-start">
                                        ...
                                        <div class="dropdown-content">
                                            <ul class="dropdown-list">
                                                <li class="dropdown-list-item">
                                                    <a href="{{ route('submenus.edit', $submenu->id) }}" class="dropdown-link">
                                                ...
                                                <li class="dropdown-list-item">
                                                    <a href="javascript:void(0)"
                                                       class="dropdown-link"
                                                       data-toggle="modal"
                                                       data-target="#deleteModal-{{ $submenu->id }}">
                                    ...
                                    <div class="modal modal-centered" id="deleteModal-{{ $submenu->id }}">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer flex justify-center">
                                                    <div class="flex items-center justify-center gap-4">
                                                        ...
                                                        <form method="POST" action="{{ route('submenus.destroy', $submenu) }}">
                                                            @csrf
                                                            @method('DELETE')
        ...
        <!-- submenu Pagination Starts -->
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row">
            <p class="text-xs font-normal text-slate-400">
                Mostrando {{ $submenus->firstItem() }} a {{ $submenus->lastItem() }} de {{ $submenus->total() }} resultados
            </p>
            {{ $submenus->appends(request()->query())->links('vendor.pagination.custom') }}
        </div>
        <!-- submenu Pagination Ends -->
    </div>
    <!-- submenu List Ends -->
</x-app-layout>
