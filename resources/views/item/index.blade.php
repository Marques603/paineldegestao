    ...
    <div class="space-y-4">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="w-[15%] uppercase">Tipo Material</th>
                        <th class="w-[15%] uppercase">Tipo Utilização</th>
                        <th class="w-[25%] uppercase">Descrição</th>
                        <th class="w-[20%] uppercase">Link Exemplo</th>
                        <th class="w-[10%] uppercase">Imagem</th>
                ...
                <tbody>
                    @foreach($itens as $item)
                        <tr>
                            ...
                            <td>{{ $item->id }}</td>
                            <td>{{ ucfirst(str_replace('_', ' ', $item->tipo_material)) }}</td>
                            <td>{{ ucfirst($item->tipo_utilizacao) }}</td>
                            <td>{{ $item->descricao }}</td>
                            <td>
                                @if($item->link_exemplo)
                                    <a href="{{ $item->link_exemplo }}" class="text-blue-600 underline" target="_blank">Ver link</a>
                                @else
                                    —
                                @endif
                            </td>
                            <td>
                                @if($item->imagem)
                                    <img src="{{ asset('storage/' . $item->imagem) }}" alt="Imagem" class="w-10 h-10 object-cover rounded">
                                @else
                                    —
                                @endif
                            </td>
                                        ...
                                        <div class="dropdown-content">
                                            <ul class="dropdown-list">
                                                <li class="dropdown-list-item">
                                                    <a href="{{ route('item.edit', $item->id) }}" class="dropdown-link">
        ...
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row">
            <p class="text-xs font-normal text-slate-400">
                Mostrando {{ $itens->firstItem() }} a {{ $itens->lastItem() }} de {{ $itens->total() }} resultados
            </p>
            {{ $itens->appends(request()->query())->links('vendor.pagination.custom') }}
        </div>
    </div>
</x-app-layout>
